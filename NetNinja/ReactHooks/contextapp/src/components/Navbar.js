...
const Navbar = () => {
  // Destructuring
  const { isLightTheme, light, dark } = useContext(ThemeContext); //can reuse useContext as many times as wee need to
  const { isAuth, toggleAuth } = useContext(AuthContext); //useContext 'hook'
  const theme = isLightTheme ? light : dark; //is it dark or light theme
  return (
    <nav style={{ background: theme.ui, color: theme.syntax }}>
      <h1>Context App</h1>
